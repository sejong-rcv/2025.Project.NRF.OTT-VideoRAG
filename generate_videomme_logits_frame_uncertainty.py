...
from llava.model.builder_custom import load_pretrained_model
...
whisper_model = WhisperForConditionalGeneration.from_pretrained("openai/whisper-large", torch_dtype=torch.float16, device_map="auto")
...
def llava_inference(qs, video, enforce=False):
    options = ["A", "B", "C", "D"]
    ...
    if video is not None or enforce:
        with torch.no_grad():
            cont = model.generate(
                input_ids,
                images=video,
        ...
        for logit_i in logits:
            logits_options.append(logit_i[0][option_ids].detach().cpu().tolist())
        text_outputs = tokenizer.batch_decode(cont[0], skip_special_tokens=True)[0].strip()
        # (Pdb) tokenizer.batch_decode(torch.tensor([1, 2, 33, 151645]), skip_special_tokens=True)
        # ['"', '#', 'B', '']
        # (Pdb) tokenizer.batch_decode(torch.tensor([[1, 2, 33, 151645]]), skip_special_tokens=True)
        # ['"#B']
        return text_outputs, logits_options
    else:
        cont = model.generate(
            input_ids,
            images=video,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_frames_num", help="the number of frames to input the LVLMs")
    parser.add_argument("--results_path", help="the path to save the inference results") # results1111_8_LLaVA-Video-7B-Qwen2
    ...
    if not os.path.exists(file_path):
        os.mkdir(file_path)
    data_path = "/mnt/ssd2/data/dataset/Video-MME/videomme/data"
    json_file = f"{args.results_path}/{file_name}.json"
    USE_OCR = args.use_ocr
    USE_ASR = args.use_asr
    USE_DET = args.use_det
    
    manual_map = OrderedDict({
    'model.image_newline': 0, 'model.embed_tokens': 0, "model.rotary_emb": 1,
    'model.layers.0': 1, 'model.layers.1': 1, 'model.layers.2': 1, 'model.layers.3': 1, 'model.layers.4': 1,
    'model.layers.5': 1, 'model.layers.6': 1, 'model.layers.7': 1, 'model.layers.8': 1, 'model.layers.9': 1,
    'model.layers.10': 1, 'model.layers.11': 1, 'model.layers.12': 1, 'model.layers.13': 1, 'model.layers.14': 1,
    'model.layers.15': 2, 'model.layers.16': 2, 'model.layers.17': 2,
    'model.layers.18': 2, 'model.layers.19': 2, 'model.layers.20': 2, 'model.layers.21': 2, 'model.layers.22': 2,
    'model.layers.23': 2, 'model.layers.24': 2, 'model.layers.25': 2, 'model.layers.26': 2, 'model.layers.27': 2,
    'model.norm': 2,
    'model.vision_tower': 2, 'model.vision_resampler': 2, 'model.mm_projector': 2,
    'lm_head': 3
    })
    ...
    model.eval() 
    # import pdb;pdb.set_trace() # model.rotary_emb
    conv_template = "qwen_1_5"  # Make sure you use correct chat template for different models
    print("has _hf_hook? ", hasattr(model, "_hf_hook"), type(getattr(model, "_hf_hook", None)))
    print("any CPU or meta left? ", 
        [n for n,p in model.named_parameters() if p.device.type=="cpu"],
        [n for n,p in model.named_parameters() if getattr(p,"is_meta",False)],
        [n for n,b in model.model.named_buffers() if b.device.type == "cpu"])
    ...
    for item in tqdm(mme_data[index:], desc="Processing items"):
        ...
        # if not "24i4ncHuf6A" in video_path:
        #     continue
        ...
        video = image_processor.preprocess(frames, return_tensors="pt")["pixel_values"].bfloat16()
        video = [video]
        ...
        if USE_DET:
            ...
            for frame in raw_video:
                ...
                processed = clip_processor(images=frame, return_tensors="pt")["pixel_values"]
        ...
        for q_num, question in enumerate(content['questions']):
            ...
            if USE_ASR or USE_DET or USE_OCR:
                ...
                if USE_DET:
                    det_docs = []
                    try:
                        request_det = json.loads(json_request[0])["DET"]
            ...
            new_qs = qs + "Select the best answer to the following multiple-choice question based on the video and the information (if given). Respond with only the letter (A, B, C, or D) of the correct option. Question: " + question['question'] + '\n' + " ".join(question['options']) + '\nThe best answer is:'
            # base
            res, logits = llava_inference(new_qs, video)
            question['response'] = res
            question['logits'] = logits

            for sampling_frame_num in [28, 24, 20, 16]:
                sampling_frames, _, _ = process_video(video_path, sampling_frame_num, 1, force_sample=True)
                sampling_video = image_processor.preprocess(sampling_frames, return_tensors="pt")["pixel_values"].bfloat16()
                sampling_video = [sampling_video]
                sampling_res, sampling_logits = llava_inference(new_qs, sampling_video)
                question[f'response_{sampling_frame_num}'] = sampling_res
                question[f'logits_{sampling_frame_num}'] = sampling_logits
