...
device = "cuda"
# max_mem  = {i: "20GiB" for i in range(torch.cuda.device_count())}
max_mem  = {0: "20GiB", 1: "24GiB", 2: "24GiB", 3: "5GiB"}

# manual_map = {
# 'model.image_newline': 0, 'model.embed_tokens': 0, 'model.layers.0': 0, 'model.layers.1': 0, 
# 'model.layers.2': 0, 'model.layers.3': 0, 'model.layers.4': 0, 'model.layers.5': 1, 'model.layers.6': 1, 
# 'model.layers.7': 1, 'model.layers.8': 1, 'model.layers.9': 1, 'model.layers.10': 1, 'model.layers.11': 1, 
# 'model.layers.12': 1, 'model.layers.13': 1, 'model.layers.14': 1, 'model.layers.15': 1, 'model.layers.16': 1,
# 'model.layers.17': 1, 'model.layers.18': 2, 'model.layers.19': 2, 'model.layers.20': 2, 'model.layers.21': 2, 
# 'model.layers.22': 2, 'model.layers.23': 2, 'model.layers.24': 2, 'model.layers.25': 2, 'model.layers.26': 2, 
# 'model.layers.27': 2, 'model.norm': 2, 'model.vision_tower': 3, 'model.vision_resampler': 3, 
# 'model.mm_projector': 3, 'lm_head': 0
# }
...
manual_map = OrderedDict({
    'model.image_newline': 0, 'model.embed_tokens': 0,
...
# OFFLOADDIR = "/mnt/HDD1_8TB/llava_offload"
...
#         "model.safetensors", "model.safetensors.index.json", "*.safetensors",
#         "config.json", "generation_config.json", "tokenizer*",
#         "*.json", "*.txt", "*.py"
#     ],
#     local_dir=None,                   # 기본 캐시 사용 (원하면 고정 경로 지정 가능)
...
#     offload_folder="/mnt/HDD1_8TB/llava_offload",
#     dtype=torch.bfloat16,
# )
# import pdb;pdb.set_trace()
...
# json_file = f"results1110_32_long-llava/{file_name}.json"
# json_file = f"results1111_8_LLaVA-Video-7B-Qwen2/{file_name}.json"
json_file = f"result_tmp/{file_name}.json"
...
# 총 900개 영상
# tmp = [len(data_i['questions']) for data_i in mme_data] 3개씩
# [data_i['duration'] for data_i in mme_data if data_i['duration'] == "short"] 300개씩
