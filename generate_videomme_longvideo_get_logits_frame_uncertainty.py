...
def process_video(video_path, max_frames_num, fps=1, force_sample=False):
    ...
    if os.path.exists(f'images_restore/{video_name}'):
        for i in range(max_frames_num):
            file_path = f'images_restore/{video_name}/frame_{i}.png'
        ...
        if not os.path.exists(f'images_restore/{video_name}'):
            os.makedirs(f'images_restore/{video_name}')  
    ...
    return spare_frames, frames_path
...
def llava_inference(qs, raw_video, max_frames_num):
    options = ["A", "B", "C", "D"]
    option_ids = [tokenizer.encode(opt)[-1] for opt in options]

    # https://github.com/Leon1207/Video-RAG-master/issues/4#issuecomment-2561595415
    conversation_txt = [
        {
        "role": "user",
        "content": [
    ...
    for _ in range(max_frames_num):
        conversation_vid[0]['content'].append({"type": "image"})

    if raw_video == None:
        conversation_txt[0]['content'][0]['text'] = qs
        prompt = processor.apply_chat_template(conversation_txt, add_generation_prompt=True)
        inputs = processor(images=raw_video, text=prompt, return_tensors='pt').to(model.device, torch.float16)
        output = model.generate(**inputs, max_new_tokens=2048, do_sample=False)
        res = processor.decode(output[0][2:], skip_special_tokens=True).split("\nassistant\n")[-1]

        return res
    else:
        conversation_vid[0]['content'][0]['text'] = qs
        prompt = processor.apply_chat_template(conversation_vid, add_generation_prompt=True)
        inputs = processor(images=raw_video, text=prompt, return_tensors='pt').to(model.device, torch.float16)
        output = model.generate(**inputs, max_new_tokens=2048, do_sample=False, return_dict_in_generate=True, output_scores=True)
        logits = output.scores
        logits_options = []
        for logit_i in logits:
            logits_options.append(logit_i[0][option_ids].detach().cpu().tolist())
        res = processor.decode(output[0][0][2:], skip_special_tokens=True).split("\nassistant\n")[-1]
        # output = model.generate(**inputs, max_new_tokens=2048, do_sample=False)
        # res = processor.decode(output[0][2:], skip_special_tokens=True).split("\nassistant\n")[-1]

        return res, logits_options
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_id)  # len(tokenizer): 151647
    ...
    for item in tqdm(mme_data[index:], desc="Processing items"):
        ...
        content = item.copy()
        frames, frames_path = process_video(video_path, max_frames_num, 1, force_sample=True)
        ...
        for q_num, question in enumerate(content['questions']):
            ...
            if USE_ASR or USE_DET or USE_OCR:
                json_request = llava_inference(retrieve_pmt_0, None, max_frames_num)
                ...
                if USE_DET:
                    ...
                    # clip_img_feats = clip_model.get_image_features(video_tensor) # 이것도 no_grad 안으로 옮김
                    with torch.no_grad():
            ...
            qs += "Question: " + question['question'] + '\n' + " ".join(question['options']) + '\nAnswer with the option’s letter from the given choices directly, you must only output one word.'
            res, logits = llava_inference(qs, raw_video, max_frames_num)
            
            question['response'] = res
            question['logits'] = logits

            for sampling_frame_num in [28, 24, 20, 16]:
                sampling_frames, _ = process_video(video_path, sampling_frame_num, 1, force_sample=True)
                sampling_raw_video = [f for f in sampling_frames]
                sampling_res, sampling_logits = llava_inference(qs, sampling_raw_video, sampling_frame_num)
